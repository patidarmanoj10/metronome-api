...
class MetApi {
  constructor () {
    ...
    if (config.eth.enabled) {
      this.ethApi = new EthApi(this.config.eth, this.logger, () => this._initMetronome())
    }
  }

  _initMetronome () {
    this.tokenExporter = new TokenExporter(this.database, this.socket)
    this.accExporter = new ACCExporter(this.database, this.socket)
    // this.status = new Status(this.database, this.socket)
