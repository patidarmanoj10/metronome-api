...
function parseFromTo(req, res, next) {
    ...
    const { query } = req
    const birthTime = 1529280000 // in seconds
    try {

        query.to = Number.parseInt(query.to || (Date.now() / 1000))
        query.from = Number.parseInt(query.from || birthTime)
...
function auction(req, res, next) {
    ...
    Promise.all([
        ...
        const remainingSeconds = nextMidnight.diff(moment(), 'seconds')
        const duration = moment.duration(remainingSeconds, 'seconds')

        stats.timeRemaining = `${duration.hours()} hour(s) ${duration.minutes()} minute(s) ${duration.seconds()} second(s)`
